...
public class PostService {
    ...
    @Autowired
    public PostService(PostRepository postRepository, ForumRepository forumRepository, TagRepository tagRepository, CommentRepository commentRepository, MongoTemplate mongoTemplate, ModelMapper modelMapper) {
    ...
    public List<GetPostCommentsResponseDto> getCommentList(String postId) {
        ...
        for (Comment comment : comments) {
            GetPostCommentsResponseDto dto = convertToDto(comment);
        ...
        for (Comment comment : comments) {
            if (comment.getParentComment() != null) {
                ...
                if (parentDto != null) {
                    parentDto.getReplies().add(comment);
                }
            }
        }
    ...
    private GetPostCommentsResponseDto convertToDto(Comment comment) {
        Boolean isEdited = comment.getCreatedAt().isBefore(comment.getLastEditedAt());
        return new GetPostCommentsResponseDto(comment.getId(), comment.getCommentContent(), comment.getCommenter(),
