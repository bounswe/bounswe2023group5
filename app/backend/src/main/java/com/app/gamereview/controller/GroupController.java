...
public class GroupController {
    ...
    @AuthorizationRequired
    @PostMapping("/apply")
    public ResponseEntity<Boolean> applyGroup(@RequestParam String groupId,
                                              @RequestBody GroupApplicationRequestDto dto,
                                              @RequestHeader String Authorization, HttpServletRequest request) {
        User user = (User) request.getAttribute("authenticatedUser");
        Boolean applied = groupService.applyGroup(groupId, user, dto);
        return ResponseEntity.ok(applied);
    }

    @AuthorizationRequired
    @PostMapping("/review-application")
    public ResponseEntity<Boolean> reviewApplication(@RequestParam String applicationId,
                                                     @Valid @RequestBody GroupApplicationReviewDto dto,
                                                     @RequestHeader String Authorization, HttpServletRequest request) {
        User user = (User) request.getAttribute("authenticatedUser");
        Boolean reviewed = groupService.reviewApplication(applicationId, user, dto);
        return ResponseEntity.ok(reviewed);
    }

    @AuthorizationRequired
    @GetMapping("/list-applications")
    public ResponseEntity<List<GroupApplicationResponseDto>> listApplications(@RequestParam String groupId,
                                                                              @RequestHeader String Authorization, HttpServletRequest request) {
        User user = (User) request.getAttribute("authenticatedUser");
